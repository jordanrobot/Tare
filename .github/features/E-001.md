...
- Entities/value objects
  - `DimensionSignature` (internal, readonly struct): integer exponents over base dims (L, M, T, [I, Θ, N, J optional]).
...
- Naming policy
  - SI-first canonical unit selection for known signatures; alias-aware.
...
- Parsing
  ...
  - Optional: later allow constructing `Quantity(value, compositeUnit)` behind a feature flag.
...
- F-002 — Dimension signature model
  - Deliver: `DimensionSignature` struct (L,M,T initially), add/subtract, equality/comparison, hashing.
...
- Naming expectations (SI-first vs domain preferences) → Configurable preferences; default SI-first.
...
- Exact set of base dimensions at launch (L,M,T only vs include electric/thermo bases).
- Preferred naming policy knobs (SI-first default; allow imperial-first?).
- Should `Parse` accept composite units for construction in v1 or keep behind a flag?
