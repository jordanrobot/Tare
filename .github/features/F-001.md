...
**Status**: Deferred (Pending MVP validation from S-005)  
**Epic**: E-001 — Option A Hybrid Core (Dimension vectors + catalog naming)  
**Priority**: P2 (Should - conditional on S-005 findings)  
**Effort**: M (1–2 days)  
**Dependencies**: S-005 (Decimal Precision Spike) must validate need

---

## Description

**⚠️ DEFERRED**: This feature is deferred pending results from S-005 (Decimal Precision Spike). MVP implementation will use existing `decimal` type for unit factors. If S-005 demonstrates unacceptable precision loss in real-world dimensional algebra scenarios, this feature will be implemented.

---

**Original Plan** (if needed):
...
The MVP approach will:
1. Implement dimensional algebra using existing `decimal` factors (F-002 through F-007)
2. Measure actual precision loss in representative scenarios (S-005)
3. Only implement `Rational` if `decimal` proves insufficient

**Benefits**:
- Faster MVP delivery (~300-400 LOC saved)
- Validates architecture before premature optimization
- Rational can be added incrementally if needed (mechanical replacement of `decimal` → `Rational` in targeted areas)
