...
internal static class BaseUnitMap
{
    ...
    {
        { UnitTypeEnum.Scalar, "each" },        // dimensionless (base = 1)
...
```csharp
// Dimensionless/Scalar units (relative to "each" base = 1)
...
- [ ] Create `BaseUnitMap` static configuration
  ...
  - [ ] Add `UnitTypeEnum.Scalar` → "each" (dimensionless base)
...
✅ **AC-10b**: Scalar base unit ("each") defined in BaseUnitMap  
✅ **AC-10c**: Dimensionless conversions work correctly (50% = 0.5 each, 1000000 ppm = 1 each)
...
public void Resolve_Percentage_ResolvesToCorrectFactor()
{
    // Arrange
    var resolver = UnitResolver.Instance;
    var expectedFactor = 0.01m; // 1% = 0.01 of base unit "each"
...
public void Resolve_DimensionlessConversions_WorkCorrectly()
{
    ...
    // 50% = 50 * 0.01 = 0.5 each
    // 1,000,000 ppm = 1,000,000 * 0.000001 = 1 each
