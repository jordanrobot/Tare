...
var forceSignature = new DimensionSignature(1, 1, -2, 0, 0, 0, 0);  // L¹M¹T⁻²
...
- ✅ `GetSignature()` returns `DimensionSignature` for any quantity
- ✅ `IsKnownDimension()` returns true if signature is in `KnownSignatureMap`
- ✅ `GetDimensionDescription()` returns human-readable description (e.g., "Force", "Energy")
- ✅ `GetDimensionDescription()` returns null or throws for unknown dimensions (decision needed)
- ✅ Works correctly for catalog units, composite units, and dimensionless quantities

### 2. Normalization Helpers

- ✅ `ToBaseUnits()` converts to SI base representation (m, kg, s, A, K, mol, cd)
- ✅ `ToCanonical()` converts to preferred unit from `KnownSignatureMap` when known
- ✅ `ToCanonical()` preserves unit for unknown signatures (no-op behavior)
- ✅ Normalization preserves numeric value correctness (tested to precision limits)
- ✅ Both methods return new `Quantity` instances (immutability preserved)

### 3. Unit Validation Helpers

- ✅ `ContainsValidUnit(string)` static method returns true for catalog and composite units
- ✅ `ContainsValidUnit` handles both "12 in" (with factor) and "in" (unit only) formats
- ✅ `ContainsValidUnit` returns false for null, empty, malformed, or unknown units
- ✅ `ContainsValidUnit` does not throw exceptions (query semantics)
- ✅ Performance: `ContainsValidUnit` is fast (uses existing caches)

### 4. Unit Discovery Helpers

- ✅ `GetUnitsForType(UnitTypeEnum)` returns list of unit names for a given dimension type
- ✅ `GetUnitsForSignature(DimensionSignature)` returns list of compatible units for a signature
- ✅ Both methods return catalog units only (not composite units)
- ✅ Results suitable for populating UI dropdowns and selection lists
- ✅ Performance: Methods use cached type index (O(1) lookup)

### 5. Diagnostics (Optional/Deferred)

- ✅ `QuantityDiagnostics.GetCacheStatistics()` returns performance metrics
- ✅ Cache stats include hit/miss counts and rates for major caches
- ✅ Diagnostics are read-only; no ability to clear or modify caches via public API
- ⏸️ Advanced diagnostics (memory usage, operation counts) deferred to user demand

### 5. API Quality Standards

- ✅ All public methods have comprehensive XML documentation
- ✅ All methods follow .NET naming conventions (PascalCase, clear intent)
- ✅ All methods have corresponding tests using `MethodName_Condition_ExpectedResult()` pattern
- ✅ Error handling consistent with F-012 standards
...
- Future enhancement (deferred to F-014+): Allow runtime configuration of preferred units
...
public static bool ContainsValidUnit(string? input)
{
    ...
    // Use same regex pattern as Quantity constructor
    var unitsPattern = new Regex(@"([A-Za-z|\^|\-|\/|'|''|\""|*].*)", RegexOptions.Compiled);
...
- ❌ Mutable state or configuration APIs
- ❌ Culture-specific formatting (F-016)
- ❌ Custom signature map registration (future enhancement)
- ❌ **Runtime configuration of preferred units** (see Q5 below - deferred to F-014+)
- ❌ GetUnitsForSignature implementation (deferred; GetUnitsForType covers common cases)
- ❌ Advanced dimension algebra (power, root operations)
- ❌ Unit aliases management (covered by existing UnitDefinitions)
- ❌ Performance optimization (F-011 already delivered caching)
...
/// Examples: Force (L¹M¹T⁻²), Energy (L²M¹T⁻²), Pressure (L⁻¹M¹T⁻²).
/// </remarks>
public DimensionSignature GetSignature() { /* ... */ }
```
...
- ✅ I have verified that helpers preserve value object immutability
- ✅ I have confirmed consistent terminology (Signature, Canonical, Base, Known)
- ✅ I have verified SOLID principles, especially SRP and OCP
- ✅ I have validated that helpers delegate to existing domain services

### Implementation Quality Validation
- ✅ I have written comprehensive tests using `MethodName_Condition_ExpectedResult()` naming
- ✅ I have considered performance (helpers should not impact hot paths)
- ✅ I have implemented null checks where appropriate (ContainsValidUnit)
- ✅ I have documented all public APIs with XML comments
- ✅ I have verified ContainsValidUnit handles both unit-only and value+unit formats

### API Design Validation
- ✅ I have verified naming follows .NET conventions
- ✅ I have ensured all methods are focused and single-purpose
- ✅ I have validated that APIs are additive (zero breaking changes)
- ✅ I have tested all methods with catalog and composite units
- ✅ I have provided unit discovery methods suitable for UI scenarios
...
### Objective Measures
- ✅ All tests pass (100% for new APIs)
- ✅ Zero breaking changes (verified by existing tests)
- ✅ All public APIs have XML documentation
- ✅ README examples compile and run correctly

### Subjective Measures
- ✅ Helper methods reviewed and deemed useful by team
- ✅ API design reviewed and deemed intuitive
- ✅ Documentation reviewed and deemed comprehensive
...
- **B**: Add optional pretty-print (Unicode: "kg·m·s⁻²")
...
- If added, likely belongs in separate feature (F-017: Configuration API)
- Current SI-first policy serves majority of users

**Future Consideration**: If demand exists, implement in F-017 with:
...
8. **Preferred Units Configuration** (2025-11-12): Defer runtime configuration of preferred units to F-017+ based on user demand. Current SI-first policy sufficient for F-013.
